  ...
  namespace MicrostrainMIP
  {
    ...
    public:
      ...
      getDeviceInfo(void)
      {
        ...
        if (readAck(CMD_SET_BASE, CMD_BASE_GET_DEV_INFO, 1.0))
        {
          ...
          if (String::startsWith(m_model_name, "3DM-GX3-"))
          {
            m_parent->debug("device is a GX3 model");
            m_model_gx3 = true;
          }
      ...
      enableContinuousStream(bool enabled)
      {
        if (enabled)
          ...
          m_parent->debug("disabling continuous output stream");
      ...
      fillPayloadFormat(std::vector<uint8_t>& format, uint16_t rate_decimation)
      {
        ...
        if (m_model_gx3)
        {
          format[1] += addPayloadFormatDescriptor(format, rate_decimation, DD_INTERNAL_TIME);
          format[1] += addPayloadFormatDescriptor(format, rate_decimation, DD_ORIENTATION_MATRIX);
        }
        else
        {
          format[1] += addPayloadFormatDescriptor(format, rate_decimation, DD_CF_EULER_ANGLES);
        }

        format[1] += addPayloadFormatDescriptor(format, rate_decimation, DD_SCALED_ACCL_VEC);
        format[1] += addPayloadFormatDescriptor(format, rate_decimation, DD_SCALED_GYRO_VEC);
        format[1] += addPayloadFormatDescriptor(format, rate_decimation, DD_SCALED_MAGN_VEC);
      }
      ...
      {
        m_rotation_matrix.resizeAndFill(3, 3, 0.0);

        for (unsigned i = 0; i < 3; ++i)
          for (unsigned j = 0; j < 3; ++j)
            m_rotation_matrix(i, j) = rotation_matrix(i, j);
      }
    ...
    private:
      ...
      interpretSample(void)
      {
        ...
        {
          // Euler Angles.
          if (payload[offset] == DD_CF_EULER_ANGLES)
          {
